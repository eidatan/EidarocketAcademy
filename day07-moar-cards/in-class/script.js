...
var cardMsg = function (card) {
  return `${card.name} of ${card.suit}, with rank ${card.rank}.`;
};
...
var moarCardsSingleCardMain = function (input) {
  ...
  let displayCard = shuffledDeck.pop();
  console.log(displayCard);
  return cardMsg(displayCard);
};

var lowCardCloned = function (deck) {
  // Draw 2 cards from the top of the deck
...
var moarCardsLowCardMain = function (input) {
  ...
  var shuffledDeck = shuffleCards(makeDeck());
  console.log(shuffledDeck);
  var lowCardDisplay = lowCardCloned(shuffledDeck);
  console.log(lowCardDisplay);
  return lowCardDisplay;
};
